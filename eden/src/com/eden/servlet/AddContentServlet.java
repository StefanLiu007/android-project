	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (0 != part1.getSize()) {
			...
			if ("image/jpeg".equals(mime1)) {
				...
				response.getWriter().println("<script type=\"text/javascript\">window.alert(\"你上传的视频类型的不合法\");location.href=\""+request.getContextPath()+"/AddContentServlet.do\";</script>");
				return;
			}
			...
			File file2 = new File("F:\\ydy\\eden\\WebContent\\upload\\"+contentVideo);
			file1.createNewFile();
			file2.createNewFile();
		...
		if (part2.getSize()!=0) {
			...
			if ("image/jpeg".equals(mime2)) {
				...
				response.getWriter().println("<script type=\"text/javascript\">window.alert(\"你上传的图片类型的不合法\");location.href=\""+request.getContextPath()+"/AddContentServlet.do\";</script>");
				return;
			}
			...
			File file4 = new File("F:\\ydy\\eden\\WebContent\\upload\\",contentPicture);
			file3.createNewFile();
			file4.createNewFile();
		...
		boolean isSuccess= contentService.addContent(content);
		if (isSuccess) {
			response.getWriter().println("<script type=\"text/javascript\">window.alert(\"添加成功\");location.href=\""+request.getContextPath()+"/ShowContentServlet.do\";</script>");
		}else {
			response.getWriter().println("<script type=\"text/javascript\">window.alert(\"添加失败\");location.href=\""+request.getContextPath()+"/AddContentServlet.do\";</script>");
		}
	}
