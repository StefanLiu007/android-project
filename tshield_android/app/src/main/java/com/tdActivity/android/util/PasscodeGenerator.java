 ...
 * 
 * The default passcode is a 6-digit decimal code. The maximum passcode length
 * is 9 digits.
 * 
...
public class PasscodeGenerator {
	...
	interface Signer {
		/**
		 * @param data
		 *            Preimage to sign, represented as sequence of arbitrary
		 *            bytes
	 ...
	 *            8-byte integer value representing internal OTP state.
	 * @param challenge
	 *            Optional challenge as array of bytes.
	 * @return A decimal response code
	 * @throws GeneralSecurityException
	 *             If a JCE exception occur
	 */
	public String generateResponseCode(long state, byte[] challenge)
		...
		if (challenge == null) {
			...
			byte value[] = ByteBuffer.allocate(8 + challenge.length)
					...
					.put(challenge, 0, challenge.length) // Concatenate with
															// challenge.
					.array();
			return generateResponseCode(value);
		}
	 ...
	 * @return True if the timeout code is valid
	 */
	public boolean verifyTimeoutCode(String timeoutCode, long currentInterval,
			int pastIntervals, int futureIntervals)
			throws GeneralSecurityException {
		...
		// Try upto "pastIntervals" before current time, and upto
		// "futureIntervals" after.
