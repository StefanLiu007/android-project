...
import com.tdActivity.android.entity.GesturePoint;
...
public class GestureDrawline extends View {
	...
	private Map<String, GesturePoint> autoCheckPointMap;// 自动选中的情况点,用来演示的
	...
	public GestureDrawline(Context context, List<GesturePoint> list, boolean isVerify, String passWord,
		...
		paint.setColor(Color.rgb(0, 151, 245));// 设置默认连线颜色
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		case MotionEvent.ACTION_MOVE:
			...
			if (pointAt == null || currentPoint.equals(pointAt)//不在圆的范围之内
					|| Constant.POINT_STATE_SELECTED == pointAt.getPointState()) {
				// 点击移动区域不在圆的区域，或者当前点击的点与当前移动到的点的位置相同，或者当前点击的点处于选中状态
				// 那么以当前的点中心为起点，以手指移动位置为终点画线
				canvas.drawLine(currentPoint.getCenterX(), currentPoint.getCenterY(), event.getX(), event.getY(),
						paint);// 画线
			} else {
				// 如果当前点击的点与当前移动到的点的位置不同
				// 那么以前前点的中心为起点，以手移动到的点的位置画线
				canvas.drawLine(currentPoint.getCenterX(), currentPoint.getCenterY(), pointAt.getCenterX(),
				...
				if (betweenPoint != null && Constant.POINT_STATE_SELECTED != betweenPoint.getPointState()) {
					// 存在中间点并且没有被选中
					Pair<GesturePoint, GesturePoint> pair1 = new Pair<GesturePoint, GesturePoint>(currentPoint,
							betweenPoint);
					lineList.add(pair1);
...
//			}
	...
	private void clearScreenAndDrawList() {
		...
		for (Pair<GesturePoint, GesturePoint> pair : lineList) {
			canvas.drawLine(pair.first.getCenterX(), pair.first.getCenterY(), pair.second.getCenterX(),
					pair.second.getCenterY(), paint);// 画线
		}
	}
	 ...
	 * 校验错误/两次绘制不一致提示
	 */
	private void drawSucessPathTip() {
		canvas.drawColor(Color.TRANSPARENT, PorterDuff.Mode.CLEAR);
		paint.setColor(Color.rgb(18, 204, 98));// 设置默认线路颜色
		for (Pair<GesturePoint, GesturePoint> pair : lineList) {
			pair.first.setPointState(Constant.POINT_STATE_SUCESS);
			pair.second.setPointState(Constant.POINT_STATE_SUCESS);
			canvas.drawLine(pair.first.getCenterX(), pair.first.getCenterY(), pair.second.getCenterX(),
					pair.second.getCenterY(), paint);// 画线
		}
		invalidate();
	...
	private void drawErrorPathTip() {
		canvas.drawColor(Color.TRANSPARENT, PorterDuff.Mode.CLEAR);
		paint.setColor(Color.rgb(225, 62, 55));// 设置默认线路颜色
		for (Pair<GesturePoint, GesturePoint> pair : lineList) {
