...
public class SwipeMenuRecyclerView extends RecyclerView {
	...
	public boolean dispatchTouchEvent(MotionEvent ev) {
		if(MainActivity.accountFragment !=null&&!MainActivity.accountFragment .isHidden()&&MainActivity.mMenu.isOpen){
			return false;
		}
	...
	public boolean onInterceptTouchEvent(MotionEvent ev) {
		...
		case MotionEvent.ACTION_DOWN:
			...
			mDownY = ev.getY();
			mTouchState = TOUCH_STATE_NONE;
			mTouchPosition = getChildAdapterPosition(findChildViewUnder(
			...
			// is not touched the opened menu view, so we intercept this touch
			// event
			if (mTouchPosition != oldPos && mTouchView != null
					&& mTouchView.isOpen()) {
		...
		case MotionEvent.ACTION_UP:
			...
			boolean isOutDuration = clickDuration > ViewConfiguration
					.getLongPressTimeout();
			...
			if (isOutDuration || isOutX || isOutY) {
				...
				if (upView instanceof SwipeMenuLayout) {
					...
					final float translationX = ViewCompat
							.getTranslationX(menuView);
					final float translationY = ViewCompat
							.getTranslationY(menuView);
					// intercept the up event when touched on the contentView of
					// the opened SwipeMenuLayout
					if (!(x >= menuView.getLeft() + translationX
							&& x <= menuView.getRight() + translationX
							&& y >= menuView.getTop() + translationY && y <= menuView
							.getBottom() + translationY)
							&& isCloseOnUpEvent) {
						return true;
					}
				}
