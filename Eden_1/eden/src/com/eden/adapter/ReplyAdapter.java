...
public class ReplyAdapter extends BaseAdapter {
	...
	private List<AnswerEden> list;		
	private LayoutInflater inflater;
	private TextView replyContent;
	private SpannableString ss;
	private Context context;
	public ReplyAdapter(Context context,List<AnswerEden> list
			,int resourceId){
		this.list = list;
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		AnswerEden bean = list.get(position);
		convertView = inflater.inflate(resourceId, null);
		replyContent = (TextView) 
					convertView.findViewById(R.id.replyContent1);
		
		final String replyNickName = bean.getAnswer().getAnswerReplyAccount();
		final String commentNickName = bean.getUse().getUserName();
		String replyContentStr = bean.getAnswer().getContent();
